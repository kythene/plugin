...
4. **`list_collections`** (optionally) to see what has been published into the space
   recently, and **`get_presence`** to see who/what is active right now and whether
   anyone is touching the same area (conflict awareness).

## First session in a fresh workspace

If `recall` AND `list_collections` both come back empty, this workspace is brand new -
...
3. `create_collection` one small, real artifact they already have to hand (a README
...
the link: `recall` returns each result's `ref`, and `catch_up` / `get_inbox` return a
